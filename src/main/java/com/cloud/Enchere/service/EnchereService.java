...
public class EnchereService {
    ...
    public List<Enchere> getExpiredByUser(int userId) throws ClassNotFoundException, SQLException {
        ...
        try {
            ...
            stmt = connection.prepareStatement("select * from enchere_global where status=2 and idutilisateur=? and notified=0");
            stmt.setInt(1, userId);
            rs = stmt.executeQuery();
            ...
            while(rs.next()) {
                ...
                categorie.setNom(rs.getString("nomcategorie"));
                e.setCategorie(categorie);
                    stmt = connection.prepareStatement("select enchere.idenchere,mouvement.montant,"+
                            "utilisateur.username from mouvement join enchere on mouvement.idenchere=enchere.idenchere "+
                            "join utilisateur on utilisateur.idutilisateur=enchere.idutilisateur where mouvement.status=2 "+
                            "and enchere.idenchere=?");
                    stmt.setInt(1, e.getId());
                    rs2 = stmt.executeQuery();
                    if(rs2.next()) {
                        e.setWinner(rs2.getString("username"));
                        e.setLast_price(rs2.getDouble("montant"));
                    }
                expired.add(e);
            }
		} catch (ClassNotFoundException | SQLException e) {
    ...
    private void expire(Connection connection, PreparedStatement stmt, ResultSet rs, int enchereId, int userId) throws SQLException {
        try {
            ...
            stmt = connection.prepareStatement("update enchere set status=2,notified=1 where idenchere=?");
